...
//            });
	...
	public function edit($id)
	{
        ...
        $admin = Admin::find($id);
        return View::make('admins.form')->with('admin', $admin);
	}
    ...
    public function login() {
		...
		$redir_url = Session::get('redir_url', 'admin');
		$asociatie = Session::get('asociatie');
		if(empty($asociatie)) {
			if(Auth::user()->type == 'super') {
			...
			Session::put('asociatie', $asociatie);
		}
	...
	public function logout() {
		Auth::logout();
		Session::forget('redir_url');
		return Redirect::action('AdminsController@login')->with('flash_info', 'Logged out.');
	}
